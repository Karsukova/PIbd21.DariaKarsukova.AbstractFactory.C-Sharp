...
namespace AbstractFactoryServiceImplementDataBase.Implementations
{
    public class ReptServiceDB : IReptService
    {
        ...
        public ReptServiceDB(AbstractDbContext context)
        {
        ...
        public void SaveZBIPrice(ReptBindingModel model)
        {
            ...
            try
            {
               ...
               missing);
                ...
                var zbis = context.ZBIs.ToList();

                //создаем таблицу
                var paragraphTable = document.Paragraphs.Add(Type.Missing);
                var rangeTable = paragraphTable.Range;
                var table = document.Tables.Add(rangeTable, zbis.Count, 2, ref
               missing, ref missing);
                ...
                for (int i = 0; i < zbis.Count; ++i)
                {
                    table.Cell(i + 1, 1).Range.Text = zbis[i].ZBIName;
                    table.Cell(i + 1, 2).Range.Text = zbis[i].Price.ToString();
                }
        ...
        {
            return context.Storages.ToList().GroupJoin(
                context.StorageMaterials.Include(r => r.Material).ToList(), storage => storage,
                storageMaterial => storageMaterial.Storage,
                    (storage, storageMatList) =>
                        new StoragesLoadViewModel
                        {
                            StorageName = storage.StorageName,
                            TotalCount = storageMatList.Sum(r => r.Count),
                            Materials = storageMatList.Select(r => new Tuple<string,
                           int>(r.Material.MaterialName, r.Count))
                        })
 .ToList();
        }
                    ...
                    foreach (var elem in dict)
                    {
                        ...
                        if (elem.Materials.Count() > 0)
                        {
                            ...
                            var excelBorder =
                             ...
                             excelcells.get_Offset(elem.Materials.Count()
                            - 1, 1));
        ...
        public List<CustomerOrdersViewModel> GetCustomerOrders(ReptBindingModel model)
        {
